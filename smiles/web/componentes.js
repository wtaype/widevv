...
// ── DATA ──────────────────────────────────────────────────────
const componentes = [
  { id:'notificaciones', icon:'fa-bell',            color:'#FF5C69', nombre:'Notificaciones',  ver:'v10.1', desc:'Toasts elegantes con 4 tipos: success, error, warning, info' },
  { id:'mensajes',       icon:'fa-message',         color:'#0EBEFF', nombre:'Mensajes',         ver:'v10.1', desc:'Alertas tipo banner centradas con auto-cierre' },
  { id:'tooltips',       icon:'fa-comment-dots',    color:'#29C72E', nombre:'Tooltips',          ver:'v11.0', desc:'Tooltips dinámicos con colores por tipo y posición inteligente' },
  { id:'modales',        icon:'fa-window-restore',  color:'#7000FF', nombre:'Modales',           ver:'v10.4', desc:'Sistema completo de modales con backdrop, animaciones y accesibilidad' },
  { id:'spinner',        icon:'fa-spinner',         color:'#FFB800', nombre:'Spinner',           ver:'v10.2', desc:'Loading elegante en botones con estado disabled automático' },
  { id:'codigo',         icon:'fa-code',            color:'#00D4FF', nombre:'Bloques de Código', ver:'v1.0',  desc:'Syntax highlight con Prism.js + botón copiar integrado' },
  { id:'animaciones',    icon:'fa-wand-magic-sparkles', color:'#FF5C69', nombre:'Animaciones',  ver:'v12',   desc:'Animaciones al scroll con IntersectionObserver inteligente' },
  { id:'copiar',         icon:'fa-copy',            color:'#29C72E', nombre:'Copiar Texto',      ver:'v10.2', desc:'Copia cualquier texto al portapapeles con un click' },
];

const code = {
notificacion: `// 4 tipos: success, error, warning, info
Notificacion('¡Guardado!', 'success');
Notificacion('Algo salió mal', 'error');
Notificacion('Ten cuidado', 'warning', 5000);`,

mensaje: `// Alerta centrada con auto-cierre
...
// Vía atributo HTML
<button \${wiTip('Tooltip mágico')}>Hover</button>`,

modal: `// HTML del modal
...
spinner: `// Activar loading
wiSpin('#miBtn');

// Desactivar (tras la operación)
wiSpin('#miBtn', false);`,

codigo: `// Wrap automático de bloques <pre>
wiCode('pre code');

// Se agrega botón copiar y highlight`,

animaciones: `// Fade up con stagger
wiVista('.cards', null, {
  anim: 'wi_fadeUp',
  stagger: 100
});
...
// ── PLANTILLAS ────────────────────────────────────────────────
const tplCard = c => `
  <div class="cp_card" id="cp_${c.id}" style="--cc:${c.color}">
    <div class="cp_card_head">
      <div class="cp_card_ico"><i class="fas ${c.icon}"></i></div>
      <div class="cp_card_info">
        <h3>${c.nombre}</h3>
        <span class="cp_badge">${c.ver}</span>
      </div>
    </div>
    <p class="cp_card_desc">${c.desc}</p>
    <div class="cp_demo" id="demo_${c.id}"></div>
    <div class="cp_code_wrap">
      <div class="cp_code_head">
        <span><i class="fas fa-code"></i> Código</span>
        <span class="cp_lang">JS</span>
      </div>
      <pre><code class="language-js">${code[c.id.replace('es','').replace('iones','ion').replace('ajes','aje')] || ''}</code></pre>
    </div>
  </div>`;

const tplNav = c => `<a href="#cp_${c.id}" class="cp_nav_item" style="--cc:${c.color}"><i class="fas ${c.icon}"></i> ${c.nombre}</a>`;
...
<div class="cp_wrap">
  ...
  <section class="cp_hero">
    ...
    <div class="cp_hero_count">
      <span class="cp_count_n">${componentes.length}</span>
      <span class="cp_count_l">Componentes</span>
    </div>
  </section>

  <nav class="cp_nav">${componentes.map(tplNav).join('')}</nav>

  <div class="cp_grid">${componentes.map(tplCard).join('')}</div>
...
const demos = {
  ...
  tooltips: () => `
    <div class="cp_demo_row">
      <button class="cp_btn cp_tip" ${wiTip('Tooltip por defecto')}>Default</button>
      <button class="cp_btn cp_tip" ${wiTip('¡Éxito!', null, 'success')}>Success</button>
      <button class="cp_btn cp_tip" ${wiTip('¡Cuidado!', null, 'warning')}>Warning</button>
      <button class="cp_btn cp_tip" ${wiTip('Info útil', null, 'info')}>Info</button>
    </div>`,
...
console.log(saludo);</code></pre>
  ...
  animaciones: () => `
    ...
    <button class="cp_btn" data-demo="anim" style="margin-top:1vh"><i class="fas fa-rotate"></i> Repetir</button>`,
...
export const init = () => {
  ...
  // Eventos demos
  $(document).on('click.comp', '[data-demo="noti"]', function() {
    const t = $(this).data('tipo');
    Notificacion(`Notificación tipo ${t}`, t);
  }).on('click.comp', '[data-demo="msg"]', function() {
    const t = $(this).data('tipo');
    Mensaje(`Mensaje tipo ${t} ✨`, t);
  }).on('click.comp', '[data-demo="modal"]', () => {
    abrirModal('cpModal');
  }).on('click.comp', '[data-demo="spin"]', function() {
    const $b = $(this);
    wiSpin($b);
    setTimeout(() => wiSpin($b, false, 'Click para cargar'), 2000);
  }).on('click.comp', '[data-demo="anim"]', () => {
    $('.cp_anim_box').removeClass('wi_visible');
    setTimeout(() => wiVista('.cp_anim_box', null, { anim:'', stagger:150 }), 50);
    $('.cp_anim_box').each(function(i) {
      setTimeout(() => $(this).addClass('wi_visible'), 150 * i);
    });
  }).on('click.comp', '[data-demo="copy"]', function() {
    wicopy($('.cp_copy_txt').text(), this, '¡Copiado!');
  });

  // Nav smooth scroll
  $(document).on('click.comp', '.cp_nav_item', function(e) {
    e.preventDefault();
  ...
  wiVista('.cp_hero_left', null, { anim:'wi_fadeLeft' });
  wiVista('.cp_cta', null, { anim:'wi_fadeUp' });

  console.log(`🧩 ${app} ${version} · Componentes OK`);
};

export const cleanup = () => {
  $(document).off('.comp');
  console.log('🧹 Componentes limpiado');
};