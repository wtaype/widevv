import{j as a}from"./vendor-gzd0YkcT.js";import{M as n,w as d,k as r}from"./main-DsMaKeNy.js";const i="#14B8A6",t={id:"mensaje",fn:n,nom:"Mensajes de Alerta",icon:"fa-comment-alt",color:i,desc:"Muestra un alert inline con 4 tipos. Reemplaza el anterior automaticamente. Sin apilacion.",code:`Mensaje('Datos guardados', 'success');
Mensaje('Campos requeridos', 'error');`,demo:()=>`<div class="cp_demo_row" style="flex-direction:column;gap:.8vh">
  ...
  </div>`,main:()=>{window.Mensaje=n}},c=[{id:"tipos",titulo:"Tipos disponibles",desc:"Cuatro tipos: success, error, warning, info. El mensaje anterior se elimina antes de mostrar el nuevo.",html:'&lt;div id="contenedor"&gt;&lt;/div&gt;',js:`import { Mensaje } from './widev.js';
...
Mensaje('¡Listo!', 'success'); // ← este gana`,demo:()=>`<div class="cp_demo_row">
  ...
  <div class="doc_code_blk">
    ...
    <pre id="${e}"><code class="language-${s}">${l}</code></pre>
    <button class="doc_copy" data-pre="${e}" title="Copiar ${s.toUpperCase()}"><i class="fas fa-copy"></i></button>
  </div>`,m=s=>`
  <section class="doc_sec" id="${s.id}">
    <h2 class="doc_h2">${s.titulo}</h2>
    <p class="doc_p">${s.desc}</p>
    ${s.html?`<div class="doc_tabs">${o("html",`dp_h_${s.id}`,s.html)}${o("js",`dp_j_${s.id}`,s.js)}</div>`:`<div class="doc_tabs">${o("js",`dp_j_${s.id}`,s.js)}</div>`}
    <div class="doc_demo">
...
<div class="doc_wrap">
  <aside class="doc_side">
    ...
    <nav class="doc_nav">${c.map((s,e)=>`<a href="#${s.id}" class="doc_nav_a"><span class="doc_nav_num">${e+1}</span>${s.titulo}</a>`).join("")}</nav>
  </aside>
  <main class="doc_main">
    <div class="doc_hero">
...
</div>`,j=()=>{window.Mensaje=n,window.Prism&&Prism.highlightAll(),a(document).on("click.msj",".doc_nav_a",function(s){s.preventDefault();const e=a(a(this).attr("href"));e.length&&a("html,body").animate({scrollTop:e.offset().top-80},300),a(".doc_nav_a").removeClass("active"),a(this).addClass("active")}).on("click.msj",".doc_copy",function(){d(a(`#${a(this).data("pre")}`).text(),this,"¡Copiado!")}),r(c.map(s=>s.id),".doc_nav_a")},f=()=>{a(document).off(".msj"),delete window.Mensaje};export{f as cleanup,j as init,_ as render,t as wi};
